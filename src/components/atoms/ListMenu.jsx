...
export default function ListMenu({ data, fetchData }) {
  ...
  return (
    <tr>
      <td>
        <img src={data.image || DEFAULT_FOTO_MAKANAN} alt="" className="w-[108px] h-[108px] rounded-3xl mb-2" />
      </td>
      <td>{data.name}</td>
      <td>{formatCategory(data.categoryid)}</td>
      <td>Rp {formatCurrency(data.price)}</td>
      <td className="flex items-center mt-10">
        <Switch checked={isChecked} onChange={(e) => handleUpdateStatus(data.id, e.currentTarget.checked)} color="red" />
        <p className="text-sm ml-2">{isChecked ? 'Tersedia' : 'Kosong'}</p>

        <div className="relative flex">
          <img src={DOTS_THREE} onClick={() => setIsOpen(!isOpen)} className="ml-5 w-8 cursor-pointer rounded-full" />
          <div className={`flex flex-col w-28 bg-white rounded-sm shadow-md overflow-hidden absolute -right-32 -top-7 ${isOpen ? 'block' : 'hidden'}`} ref={ref}>
            <button className="hover:bg-orange hover:text-white px-5 py-2" onClick={() => navigate(`/admin/editmenu/${data.id}`)}>
              Edit
            </button>
